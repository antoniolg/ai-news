...
test("renders the scaffold homepage", async ({ page }) => {
  ...
  await expect(page.getByText("Gemini genera archivos descargables desde el chat")).toBeVisible();
  await expect(page.getByRole("link", { name: /Leer fuente/ })).toHaveCount(7);
});

test("filters timeline news from the search input", async ({ page }) => {
  await page.goto("/");

  await page.getByLabel("Buscar noticias").fill("Gemini");
