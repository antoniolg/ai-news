...
test("renders the scaffold homepage", async ({ page }) => {
  ...
  await expect(page.getByText("Codex empieza a trabajar directamente en Chrome")).toBeVisible();
  await expect(page.getByRole("link", { name: /Leer fuente/ })).toHaveCount(22);
});
